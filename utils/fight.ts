...
export const fight = (
  ...
  do {
    ...
    log.push(
      `${attacker.warrior.name} will attack ${defender.warrior.name} with strenght of ${attackStrength}`
    );

    if (defender.dp + defender.warrior.agility > attackStrength) {
      log.push(
        `${defender.warrior.name} defend himself from ${attacker.warrior.name} attack.`
      );
      ...
      if (defender.dp < 0) {
        log.push(
          `${attacker.warrior.name} witth strong attack destroyed shield ${
            defender.warrior.name
          } and give him ${-defender.dp} damage.`
        );
      ...
      log.push(
        `${attacker.warrior.name} give ${attackStrength} damage ${defender.warrior.name}.`
      );
